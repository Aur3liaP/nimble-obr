    ...
    <div className="rounded-xl border border-stone-700/60 bg-stone-900/50 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold uppercase tracking-widest text-stone-400">Free Roll</span>
        </div>
        ...
        <div className="px-3 pb-3 flex flex-col gap-3 border-t border-stone-800">
          ...
          <div className="grid grid-cols-4 gap-1.5 pt-2">
            {DICE.map((die) => (
              <button
                ...
                className={`flex flex-col items-center py-2 rounded-lg border font-bold text-xs transition-all active:scale-95 hover:brightness-125 ${die.color} ${die.bg} ${die.border}`}
              >
          ...
          <div className="flex items-center gap-4 flex-wrap">
            <div className="flex items-center gap-1.5">
              ...
              <button onClick={() => setCount((c) => Math.max(1, c - 1))} className="w-5 h-5 rounded bg-stone-700 text-stone-300 text-xs font-bold hover:bg-stone-600">−</button>
              <span className="text-xs font-bold text-stone-200 w-4 text-center">{count}</span>
              <button onClick={() => setCount((c) => Math.min(10, c + 1))} className="w-5 h-5 rounded bg-stone-700 text-stone-300 text-xs font-bold hover:bg-stone-600">+</button>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-stone-500 uppercase">Mod</span>
              <button onClick={() => setModifier((m) => m - 1)} className="w-5 h-5 rounded bg-stone-700 text-stone-300 text-xs font-bold hover:bg-stone-600">−</button>
          ...
          <div className="flex gap-1.5">
            {(["standard", "advantage", "disadvantage"] as RollMode[]).map((m) => (
              <button
                key={m}
                onClick={() => setMode(m)}
                className={`flex-1 py-1.5 px-1 rounded-lg text-[10px] font-semibold border transition-all ${
                  mode === m
                    ? m === "advantage" ? "border-emerald-500 bg-emerald-900/50 text-emerald-300"
                    : m === "disadvantage" ? "border-rose-500 bg-rose-900/50 text-rose-300"
          ...
          {mode !== "standard" && (
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setExtraCount((c) => Math.max(1, c - 1))} className="w-5 h-5 rounded bg-stone-700 text-stone-300 text-xs font-bold hover:bg-stone-600">−</button>
              <span className="text-xs font-bold text-stone-200 w-4 text-center">{extraCount}</span>
              <button onClick={() => setExtraCount((c) => Math.min(5, c + 1))} className="w-5 h-5 rounded bg-stone-700 text-stone-300 text-xs font-bold hover:bg-stone-600">+</button>
              <span className="text-[10px] text-stone-500">{mode === "advantage" ? "keep highest" : "keep lowest"}</span>
            </div>
          )}
...
function dieFace(sides: number): string {
  const faces: Record<number, string> = { 4: "△", 6: "⬡", 8: "◇", 10: "⬟", 12: "⬠", 20: "⬣", 100: "%" };
  return faces[sides] ?? "◻";
}
